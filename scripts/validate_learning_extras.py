...
reading_header_js = (root / 'docs/assets/reading-header.js').read_text(encoding='utf-8')
errors = []

for asset in ('./assets/chapter-meta.js', './assets/chapter-meta.css', './assets/learning-tools.js', './assets/learning-tools.css', './assets/reading-header.js'):
    if asset not in index:
        errors.append(f'index.html não carrega {asset}')

for marker in ('>Tempo<', '>Nível<', '>Antes<', 'prerequisites', 'hero.after(box)'):
...
for marker in ('clarity-intro', 'source-lead', 'data-reading-header-hidden', 'main.dataset.clarityLesson'):
    if marker not in reading_header_js:
        errors.append(f'reading-header.js incompleto: {marker}')
...
print('Metadados compactos, abertura sem duplicação, busca, grafo, notas e níveis de domínio válidos.')
